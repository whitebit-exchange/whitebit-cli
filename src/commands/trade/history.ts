...
export const tradeHistoryCommand = defineCommand({
  name: 'history',
  description: 'Get complete trading history with optional filtering and pagination',
  options: {
  ...
  handler: async ({ flags }) => {
    ...
    const response = await api.tradesHistory({
      market: flags.market,
      limit: flags.limit,
      offset: flags.offset,
    });
